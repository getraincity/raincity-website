...
/**
 * The interactive half of the site header.
 *
 * Everything here used to live in `Header.tsx` with `"use client"` at the top
 * and `import { nav } from "@/lib/content"` under it. That single import was
 * the most expensive line in the project: `nav` is built from `services.map`
 * and `locations.map`, so importing it pulled the whole of `content.ts` —
 * every service's scope copy, every community's page body, every blog article
 * — into the client bundle, on every route, duplicating text that is already
 * in the server-rendered HTML. `Photo` did the same for the 1,200-line photo
 * registry.
 *
 * The fix is the App Router's own answer: the server resolves the data and
 * passes it down as plain props. Nothing below imports a data module, only
 * types, and the props are the handful of labels, hrefs and photo fields the
 * header genuinely renders. Keep it that way — reaching back into
 * `@/lib/content` from this file re-opens the whole leak.
 */

/** One entry under a dropdown. `photo` is resolved server-side, or null. */
export type HeaderNavChild = {
  label: string;
  href: string;
  photo: PhotoFrameData | null;
};
...
export type HeaderSocial = { label: string; href: string; icon: string };
...
export function HeaderClient({
  ...
  social: readonly HeaderSocial[];
}) {
  const [openMenu, setOpenMenu] = useState<string | null>(null);
  const [preview, setPreview] = useState<PhotoFrameData | null>(null);
...
function DesktopNavItem({
  ...
  onToggle: () => void;
  onOpen: () => void;
  preview: PhotoFrameData | null;
  setPreview: (p: PhotoFrameData | null) => void;
}) {
            ...
            {withPhotos ? (
              <div className="grid grid-cols-12 gap-6">
                  ...
                  {item.children.map((child) => (
                    <li key={child.label}>
                      <Link
                        ...
                        onMouseEnter={() => setPreview(child.photo)}
                      >
                        {child.label}
                      </Link>
                    </li>
                ...
                <div className="col-span-4">
                  <PhotoFrame
                    photo={preview ?? item.children[0].photo!}
                    ratio="16:10"
                    sizes="380px"
                  />
